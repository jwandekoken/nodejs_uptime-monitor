...
cli.responders.help = () => {
  const commands = {
    ...
    'list logs':
      'Show a list of all the files available to be read (compressed only)',
...
cli.responders.moreUserInfo = (str) => {
  ...
  // ex: str = "more user info --99999"
  // str.split("--") = ["more user info ", "99999"]
  const arr = str.split('--');
  const userId =
    ...
    if (!err && checkIds && checkIds.length > 0) {
      ...
      checkIds.forEach((checkId) => {
        _data.read('checks', checkId, (err, checkData) => {
          ...
          // get the state of the check, default to unkown
          let stateOrUnkown =
...
cli.responders.moreCheckInfo = (str) => {
  // get the ID from the string
  // ex: str = "more check info --99999"
  // str.split("--") = ["more check info ", "99999"]
  const arr = str.split('--');
  const checkId =
    typeof arr[1] == 'string' && arr[1].trim().length > 0
      ? arr[1].trim()
...
cli.responders.moreLogInfo = (str) => {
  ...
  // ex: str = "more log info --99999"
  // str.split("--") = ["more log info ", "99999"]
  const arr = str.split('--');
  const logFilename =
    typeof arr[1] == 'string' && arr[1].trim().length > 0
      ? arr[1].trim()
